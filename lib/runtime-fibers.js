...
var pushFrame = require('./frame').pushFrame;
...
function framed(file, line, fn) {
	return function() {
		var frame = pushFrame(file, line, fn.name);
		try {
...
exports.await = function(file, line, object, property, index1, index2, returnArray) {
	...
	if (wrapper) {
		...
		return g.emitter ? framed(file, line, wrapper) : wrapper;
	}
	counters.slowAwait++;
	...
	wrapper = function() {
		...
		var frame = g.emitter && pushFrame(file, line, fn.name);
		fn.apply(that, args);
...
exports.async = function(fn, index) {
	...
	return asyncTemplate(fn.length)(fn, index);
}
