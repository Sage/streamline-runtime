...
var getTag = function getTag(options, index) {
	if (options && typeof options === "object") {
			if (options.returnArray) return "A" + index;
			if (options.returnObject) return "O" + options.returnObject.join('/') + index;
		}
	return index;
};

var invoke = function invoke(that, fn, args, index, options) {
	...
	if (options.errbackIndex != null) {
			...
			args[index == null ? args.length : index] = callback;
		}
	fn.apply(that, args);
	sync = false;
	return result;
...
var starTemplate = function (fn, options) {
	...
	//if (index == null) index = fn.length - 1;
	var F = regeneratorRuntime.mark(function callee$1$0() {
		...
		return regeneratorRuntime.wrap(function callee$1$0$(context$2$0) {
			while (1) switch (context$2$0.prev = context$2$0.next) {
				case 0:
					context$2$0.next = 2;
					return invoke(this, fn, args$2$0, index, options);
